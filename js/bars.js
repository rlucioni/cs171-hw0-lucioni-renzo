...
var canvasHeight, canvasWidth, dataset, key, margin, svg, xScale, yScale;
...
d3.selectAll("p").on("click", function() {
  var bars, highKey, i, labels, maxValue, newNumber, numValues, paragraphID;
  ...
  labels = svg.selectAll("text").data(dataset, key);
  ...
  labels.transition().duration(500).text(function(d) {
    return d.value;
  }).attr({
    x: function(d, i) {
      return xScale(i) + xScale.rangeBand() / 2;
  ...
  bars.exit().transition().duration(500).attr("x", -xScale.rangeBand() - xScale.rangeBand() / 2).remove();
  return labels.exit().transition().duration(500).attr("x", -xScale.rangeBand() - xScale.rangeBand() / 2).remove();
});
